...
const Feather & Feather::render( ) const
{
    ...
    // Shape feather = Shape( { center + VectorX( FEATHER_LENGTH ), center + VectorY( -FEATHER_BASE_LENGTH ),
...
const Bird & Bird::render( ) const
{
    ...
    Shape wing = Shape( { Coordinate( 0.0, 0.0 ), Coordinate( -24.0, sin( world( )->age( ) / 16.0 ) * 16.0 ), Coordinate( 16.0, 0.0 ) } );
    if( velocity( ).dx( ) < 0.0 )
        wing.mirror_y( );
