...
inline Coordinate world_position_from_event( const SDL_Event & event, World * world ) { return world->active_camera( )->screen_to_world( Coordinate( event.motion.x, event.motion.y ) ); }
...
void Engine::input_eng( varray<Input *> & inputs, World * world )
{
    ...
    static int joystick_x_axis_right_new = joystick_x_axis_right;
    static int joystick_y_axis_right_new = joystick_y_axis_right;
