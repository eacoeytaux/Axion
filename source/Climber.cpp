...
void Climber::render_head( ) const
{
    static_setup( Drawing, head_drawing )
    {
        ...
        // head
        head_drawing.draw( skin_color( ), Shape::circle( HEAD_RADIUS ) );

        if( m_skin == MOHAWK )
        {
            // shaved hair
            varray<Coordinate> top_half_coordinates = Shape::circle( HEAD_RADIUS ).coordinates( );
        ...
        // ears
        head_drawing.draw( skin_color( ), Shape::circle( EAR_RADIUS, VectorX( -HEAD_RADIUS + EAR_RADIUS ) ) );
        head_drawing.draw( skin_color( ), Shape::circle( EAR_RADIUS, VectorX( HEAD_RADIUS - EAR_RADIUS ) ) );
    }
    ...
    if( m_skin == MOHAWK )
    {
        ...
        { // beard
            draw( hair_color( ), Shape::rectangle( FACE_RADIUS * 2.0, 12.0, face_center( ) + VectorY( -12.0 ) ) );
            draw( hair_color( ), Shape::rectangle( 12.0, 4.0, face_center( ) + VectorY( -6.0 ) ) );
        }
    }

    // nose
    // draw( skin_color_secondary( ), Shape::circle( NOSE_RADIUS, face_center( ) + Vector( -half( NOSE_WIDTH ), NOSE_OFFSET_Y - half( NOSE_HEIGHT ) + NOSE_RADIUS ) ) );
    // draw( skin_color_secondary( ), Shape::circle( NOSE_RADIUS, face_center( ) + Vector( half( NOSE_WIDTH ), NOSE_OFFSET_Y - half( NOSE_HEIGHT ) + NOSE_RADIUS ) ) );
    // draw( { skin_color( ), skin_color( ), skin_color_secondary( ), skin_color_secondary( ) }, Shape::rectangle( NOSE_WIDTH, NOSE_HEIGHT, face_center( ) + VectorY( NOSE_OFFSET_Y ) ) );
    // draw( skin_color_secondary( ), Shape::circle( NOSE_WIDTH, face_center( ) + VectorY( NOSE_OFFSET_Y ) ) );

    // eyes
    if( m_blink_wait.remaining( ) && m_blink_wait.remaining( ) )
    {
        draw( eye_color( ), Shape::rectangle( EYE_WIDTH, EYE_WIDTH, face_center( ) + Vector( EYE_SPACING, 0.0 ) ) );
        draw( eye_color( ), Shape::rectangle( EYE_WIDTH, EYE_WIDTH, face_center( ) + Vector( -EYE_SPACING, 0.0 ) ) );
    }
    else
    {
        draw( eye_color( ), Shape::rectangle( EYE_WIDTH, 1.0, face_center( ) + Vector( EYE_SPACING, -1.0 ) ) );
        draw( eye_color( ), Shape::rectangle( EYE_WIDTH, 1.0, face_center( ) + Vector( -EYE_SPACING, -1.0 ) ) );
    }
}
...
void Climber::render_torso( ) const
{
    ...
    // pants base
    body_drawing.draw( pants_color( ), Shape( { waist + Coordinate( PANTS_WIDTH_UPPER.half( ), 0.0 ),
                                                waist + Coordinate( -PANTS_WIDTH_UPPER.half( ), 0.0 ),
                                                waist + Coordinate( -PANTS_WIDTH_LOWER.half( ), -PANTS_HEIGHT.half( ) ),
                                                waist + Coordinate( PANTS_WIDTH_LOWER.half( ), -PANTS_HEIGHT.half( ) ) } ) );

    // undershirt
    body_drawing.draw( undershirt_color( ), Shape::rectangle( BODY_WIDTH.half( ), BODY_HEIGHT ) );
    ...
    body_drawing.draw( belt_buckle_color( ), Shape::rectangle( BELT_BUCKLE_WIDTH, BELT_BUCKLE_WIDTH, waist ) );

    // jacket
    body_drawing.draw( jacket_color( ), Shape::circle( BACK_ARM_WIDTH.half( ), back_shoulder( ) ) );
    body_drawing.draw( jacket_color( ), Shape::rectangle( BODY_WIDTH / 3.0, BODY_HEIGHT - SHOULDER_RADIUS, Coordinate( BODY_WIDTH / 3.5, -half( SHOULDER_RADIUS ) ) ) );
    body_drawing.draw( jacket_color( ), Shape::rectangle( BODY_WIDTH / 3.0, BODY_HEIGHT - SHOULDER_RADIUS, Coordinate( -BODY_WIDTH / 3.5, -half( SHOULDER_RADIUS ) ) ) );
...
void Climber::render_legs( ) const
{
    static_setup( Drawing, boot_drawing )
    {
        ...
        Shape boot_sole = Shape::rectangle( BOOT_BASE_WIDTH, BOOT_SOLE_HEIGHT, Coordinate( x_offset_base, -BOOT_SOLE_HEIGHT.half( ) ) );
        boot_drawing.draw( boot_sole_color( ), boot_sole );
        {
            Shape boot_sub_sole = Shape::rectangle( BOOT_SOLE_SUB_WIDTH, BOOT_SOLE_SUB_HEIGHT, Coordinate( x_offset_base - BOOT_BASE_WIDTH.half( ) + BOOT_SOLE_SUB_WIDTH.half( ), -BOOT_SOLE_SUB_HEIGHT - BOOT_SOLE_HEIGHT.half( ) ) );
            boot_drawing.draw( boot_sole_color( ), boot_sub_sole );
            for_range( floor( BOOT_BASE_WIDTH.half( ).half( ) ) )
            {
                Shape boot_sub_sole = Shape::rectangle( BOOT_SOLE_SUB_WIDTH, BOOT_SOLE_SUB_HEIGHT, Coordinate( x_offset_base + BOOT_BASE_WIDTH.half( ) - BOOT_SOLE_SUB_WIDTH.half( ) - ( ( BOOT_SOLE_SUB_WIDTH + BOOT_SOLE_SUB_GAP_WIDTH ) * (Planc)i ), -BOOT_SOLE_SUB_HEIGHT - BOOT_SOLE_HEIGHT.half( ) ) );
                boot_drawing.draw( boot_sole_color( ), boot_sub_sole );
            }
        }

        Shape boot_shin = Shape::rectangle( BOOT_TOP_WIDTH, BOOT_TOP_HEIGHT, Coordinate( 0.0, BOOT_TOP_HEIGHT.half( ) ) );
        Shape boot_base = Shape::rectangle( BOOT_BASE_WIDTH, BOOT_BASE_HEIGHT, Coordinate( x_offset_base, BOOT_BASE_HEIGHT.half( ) ) );
    ...
    // back leg
    {
        ...
        // draw( pants_color( ), Shape::circle( LEG_WIDTH.half( ), knee ) );
...
void Climber::render_front_arm( ) const
{
    ...
    draw( jacket_color( ), Shape::circle( SHOULDER_RADIUS, shoulder ) );

    draw( jacket_color( ), Line( lower_arm.c1( ), lower_arm.joint( ) ), (double)FRONT_ARM_WIDTH );
    draw( jacket_color( ), Shape::circle( FRONT_ARM_WIDTH.half( ), upper_arm.joint( ) ) );
...
void Climber::render_front_hand( ) const
{
    ...
    draw( skin_color( ), Shape::circle( FRONT_ARM_WIDTH.half( ), hand ) );
    draw( skin_color( ), Line( finger_base, finger_tip ), FINGER_WIDTH );
}

void Climber::render_back_hand( ) const
{
    draw( skin_color( ), Shape::circle( BACK_ARM_WIDTH.half( ), back_hand( ) ) );
}

void Climber::render_crossbow( ) const
{
    Shape crossbow_shape = Shape( { { 6.0, 0.0 }, Coordinate( 0.0, 0.0 ), Coordinate( -2.0, -2.0 ), Coordinate( -4.0, -2.0 ), Coordinate( -4.0, -4.0 ), Coordinate( 1.0, -1.0 ), Coordinate( 5.0, -1.0 ) } ).scale( 5.0 );
...
const Climber::Hook & Climber::Hook::render( ) const
{
    ...
    { // rope
        ...
        { // draw rope detail coils
            ...
            {
                Shape rope_strip_rect = Shape::rectangle( ROPE_WIDTH, ROPE_WIDTH, base - ( rope_chunk * ( i + 1 ) * 2 ), rope_chunk.angle( ) );
