...
GrassTerrain::GrassTerrain( World * world, const varray<Coordinate> & _vertices ) : Terrain( world, _vertices )
{
    ...
    varray<Shape> grass_top;
    varray<Shape> grass_bottom;
    ...
    for_each( edge, edges( ) )
    {
        ...
        draw( { DIRT_COLOR, DIRT_COLOR, BLACK, BLACK }, Shape( { v2,
                                                                 v1,
                                                                 v1 + Vector( 0, -DEPTH_COLOR_LENGTH ),
                                                                 v2 + Vector( 0, -DEPTH_COLOR_LENGTH ) } ) );

        draw( BLACK, Shape( { v2 + Vector( 0, -DEPTH_COLOR_LENGTH ),
                              v1 + Vector( 0, -DEPTH_COLOR_LENGTH ),
                              v1 + Vector( 0, -DEPTH_LENGTH ),
                              v2 + Vector( 0, -DEPTH_LENGTH ) } ) );

        auto generate_grass = [ & ]( varray<Shape> & grass )
        {
            ...
            while( true )
            {
                ...
                grass.insert_back( Shape( { v1 + v2 + base_offset,
                                            v1 + v2,
                                            v1 + v3,
                                            v1 + v3 + base_offset,
    ...
    for_each( vertex, vertices( ) )
    {
        ...
        grass_top.insert_back( Shape( { base,
                                        intersect1,
        ...
        grass_bottom.insert_back( Shape( { base,
                                           intersect1,
                                           intersect1 + VectorA( base_offset1, GRASS_BASE ),
    ...
    for_each( shape, grass_bottom ) draw( GRASS_COLOR_DARK, Shape::expand( shape, OUTLINE_THICKNESS ) );
    for_each( shape, grass_top ) draw( GRASS_COLOR_DARK, Shape::expand( shape, OUTLINE_THICKNESS ) );
    for_each( shape, grass_top ) draw( GRASS_COLOR_LIGHT, shape );
}
