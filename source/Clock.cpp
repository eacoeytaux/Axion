...
string Clock::timestamp( const string _delim ) const
{
    ...
    {
        static string ret = "00" + _delim + "00" + _delim + "00";
        return ret;
    }
    return string( time_str );
}
...
string Clock::datestamp( const string _delim ) const
{
    ...
    {
        static string ret = "00" + _delim + "00" + _delim + "0000";
        return ret;
    }
}

uint Clock::total_ms( ) const { return (uint)( m_ms ); }
uint Clock::seconds( ) const { return (uint)( m_ms / 1000.0 ); }
uint Clock::minutes( ) const { return (uint)( m_ms / ( 1000.0 * 60.0 ) ); }
uint Clock::hours( ) const { return (uint)( m_ms / ( 1000.0 * 60.0 * 60.0 ) ); }
