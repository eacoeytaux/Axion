#include "DistantBird.hpp"

#include "World.hpp"

using mtmercy::DistantBird;

namespace
{
const Planc SCALE = 3.0;

const Planc X_SPEED = 2.0;

const Planc WING_HEIGHT_SPAN = 6.0;
const uint WING_CYCLE = 4;

const Planc FLIGHT_HEIGHT_SPAN = 16.0;
const uint FLIGHT_HEIGHT_CYCLE = 17;

const Color BIRD_COLOR = BLACK;
} // namespace

DistantBird::DistantBird( World * world, const Coordinate & _position ) : Object( world, _position )
{
    drawing_always_dirty( true );

    z( 0.5 );
    gravity_ratio( 0.0 );
    terrain_boundaries( false );

    velocity( VectorX( X_SPEED ) );

    m_wing_cycle_offset = (uint)Random::rdouble( WING_CYCLE ) * TAU;
    m_flight_cycle_offset = (uint)Random::rdouble( FLIGHT_HEIGHT_CYCLE ) * half( PI );
}

const DistantBird & DistantBird::render( ) const
{
    Object::render( );

    Planc wing_y = WING_HEIGHT_SPAN * sin( (double)( age( ) + m_wing_cycle_offset ) / (double)WING_CYCLE );
    Planc flight_y = FLIGHT_HEIGHT_SPAN * sin( (double)( age( ) + m_flight_cycle_offset ) / (double)FLIGHT_HEIGHT_CYCLE );

    Shape bird_shape = Shape( { Coordinate( 0.0, 0.0 ), Coordinate( -1.0, 1.0 ), Coordinate( -4.0, 1.0 ), Coordinate( -12.0, -1.0 ), Coordinate( -4.0, -1.0 ), Coordinate( -2.0, 0.0 ) } );
    Shape bird_wing = Shape( { Coordinate( -2.0, 0.0 ), Coordinate( -9.0, -1.0 ), Coordinate( -5.0, wing_y ) } );

    bird_shape.scale( SCALE );
    bird_wing.scale( SCALE );

    bird_shape.move( VectorY( flight_y ) );
    bird_wing.move( VectorY( flight_y ) );

    if( velocity( ).dx( ) < 0.0 )
    {
        bird_shape.mirror_y( );
        bird_wing.mirror_y( );
    }

    draw( BIRD_COLOR, bird_shape );
    draw( BIRD_COLOR, bird_wing );

    return *this;
}
