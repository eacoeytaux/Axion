...
const Waterfall & Waterfall::render( ) const
{
    ...
    draw( WATER_COLOR, Polygon::rectangle( width( ), height( ), Coordinate( 0, height( ).half( ) ) ) );

    draw( { top_color, top_color, bottom_color, bottom_color }, Polygon::rectangle( width( ), height( ), Coordinate( 0, height( ).half( ) ) ) );

    for_each( foam, m_foam ) { draw( FOAM_COLOR, Polygon::equilateral( FOAM_EDGE_COUNT, foam.radius, foam.position ) ); }
