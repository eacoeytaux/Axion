...
void render_convex_polygon( varray<Color> cref _colors, varray<Coordinate> cref _coordinates )
{
...
void render_colored_polygon( Drawing::ColoredPolygon cref _colored_polygon, cdec _zoom )
{
    if( _colored_polygon.thickness == FILLED )
    {
        ...
        {
            varray<Coordinate> cref c = _colored_polygon.polygon.coordinates( false );
            varray<varray<uint>> cref t = _colored_polygon.polygon.triangle_indices( );

            for_range( i, t.size( ) )
            {
                varray<Color> colors = { _colored_polygon.colors[ 0 ] };

                if( _colored_polygon.colors.size( ) > 1 )
                {
                    colors = { _colored_polygon.colors[ t[ i ][ 0 ] ],
            ...
            #if defined ( AXN_DEBUG )
            // todo convex_polygon_count += 1;
            // todo convex_polygon_triangle_count += t.size( );
            #endif
        }
...
void render_nodes( varray<Drawing::Node> cref _drawing_nodes,
                   varray<Drawing::ColoredPolygon> cref _crop_polygons,
                   varray<Drawing::ColoredPolygon> cref _hole_polygons,
                   cdec _zoom )
{
    return_if( _drawing_nodes.empty( ) );

    varray<Drawing::Node> drawing_nodes = _drawing_nodes;
    ...
    varray<Drawing::ColoredPolygon> crop_polygons = _crop_polygons;
    varray<Drawing::ColoredPolygon> hole_polygons = _hole_polygons;
    ...
    if( crop_polygons.size( ) || hole_polygons.size( ) )
    {
        ...
        {
            render_colored_polygon( polygon, _zoom );
        }
    ...
    {
        render_nodes( drawing, crop_polygons, hole_polygons, _zoom );
    }

    for_each( colored_polygon, drawing_node.colored_shapes )
    {
        render_colored_polygon( colored_polygon, _zoom );
    }

    render_nodes( drawing_nodes, crop_polygons, hole_polygons, _zoom );

    ogl::pop_matrix( );
}

void render_drawing( Drawing cref _drawing, cdec _zoom )
{
    ...
    auto nodes = _drawing.nodes( );
    //for_each( node, nodes )
    {
        render_nodes( nodes, varray<Drawing::ColoredPolygon>( ), varray<Drawing::ColoredPolygon>( ), _zoom );
    }
...
void Camera::render( )
{
    ...
    cdec _screen_width = Engine::screen_width( );
    cdec _screen_height = Engine::screen_height( );

    cdec _zoom = Camera::zoom( );

    cCoordinate _camera_center = center( );
    ...
    ogl::push_matrix( );
    {
        ...
        //auto render_convex_polygon = [ & ] ( varray<Color> cref _colors, varray<Coordinate> cref _coordinates )
        //{
        //    if( !_colors.size( ) || !_coordinates.size( ) )
        //    {
        //        return;
        ...
        //auto render_colored_polygon = [ & ] ( Drawing::ColoredPolygon cref _colored_polygon )
        //{
        //    if( _colored_polygon.thickness == FILLED )
        //    {
        //        ogl::push_matrix( );
        ...
        //auto render_drawing = [ & ] ( Drawing cref _drawing )
        //{
        //    Transform cumulative_transform = _drawing.cumulative_transform( );
        ...
        //    // todo should be normal Polygons
        //    varray<Drawing::ColoredPolygon> cropped_polygons;
        //    varray<Drawing::ColoredPolygon> erased_polygons;

        //    auto nodes = _drawing.nodes( );
        //    for_each( node, nodes )
        //    {
        //        cropped_polygons += node.crop_shapes;
        //        erased_polygons += node.hole_shapes;

        //        if( cropped_polygons.size( ) || erased_polygons.size( ) )
        //        {
        //            ogl::clear_stencil( );
        ...
        //            ogl::stencil_add( );
        //            for_each( polygon, cropped_polygons )
        //            {
        //                if( polygon.polygon.convex( ) )
        //                {
        //                    render_convex_polygon( { TRANSPARENT }, polygon.polygon.coordinates( true ) );
        //                }
        //                else
        //                {
        //                    // TODO!
        ...
        //            if( cropped_polygons.size( ) )
        //            {
        //                ogl::stencil_remove( );
        //            }

        //            for_each( polygon, erased_polygons )
        //            {
        //                if( polygon.polygon.convex( ) )
        //                {
        //                    render_convex_polygon( { TRANSPARENT }, polygon.polygon.coordinates( true ) );
        ...
        //        for_each( drawing, node.drawings )
        //        {
        //            // render_drawing( drawing );
        //        }

        //        for_each( colored_polygon, node.colored_shapes )
        //        {
        //            render_colored_polygon( colored_polygon );
        //        }
        //    }
        ...
        auto render_visible = [ & ] ( Visible * visible, bool fixed )
        {
            ...
            ogl::push_matrix( );
            {
                ...
                ogl::translate( visible->center( ).x( ), visible->center( ).y( ) );

                render_drawing( *visible, _zoom );
            }
            ogl::pop_matrix( );
        };
        ...
        auto render_lighting = [ & ] ( )
        {
            if( m_world->current_room( )->lighting_active( ) )
            {
                if( const Lighting * lighting = m_world->current_room( )->lighting( ) )
                {
                    ...
                    {
                        ogl::depth_always( );
                        render_convex_polygon( { lighting->ambient_color( ) }, Polygon::rectangle( _screen_width, _screen_height ).coordinates( ) );
                    }
                    ...
                    if( _light_sources.size( ) )
                    {
                        ogl::push_matrix( );
                        {
                            ...
                            Polygon cover = Polygon::rectangle( ( _screen_width / _zoom ), ( _screen_height / _zoom ), _camera_center );
        ...
        auto render_world_bounds = [ & ] ( )
        {
            ...
            Polygon world_bounds_polygon( m_world->current_room( )->bounds( ) );
            world_bounds_polygon.move( -Vector( _camera_center ) );
            world_bounds_polygon.scale( _zoom );

            render_convex_polygon( { TRANSPARENT }, world_bounds_polygon.coordinates( ) );
            render_convex_polygon( { COLOR }, Polygon::rectangle( _screen_width, _screen_height ).coordinates( ) );
        };
...
Drawing Camera::debug_overlay_drawing( ) const
{
    ...
    cVector _target_offset = target( ) - center( );

    cPolygon _target_outer = Polygon::circle( TARGET_RADIUS + FPS_LINE_THICKNESS, _target_offset );
    cPolygon _target_inner = Polygon::circle( TARGET_RADIUS, _target_offset );
    cPolygon _target_cover = Polygon::circle( TARGET_RADIUS );

    cLine _fps_line = Line( ORIGIN, CoordinateY( FPS_RADIUS ).rotate( delta ) );
    cPolygon _fps_circle = Polygon::circle( FPS_RADIUS );
    cPolygon _fps_dot = Polygon::circle( TARGET_RADIUS );
...
void Camera::zoom( cdec _zoom )
{
    ...
    dec zoom = minmax( MIN_ZOOM, _zoom, MAX_ZOOM );
...
bool Camera::in_view( FixedRectangle cref _world_bounding_box, cdec _z ) const
{
    ...
    FixedRectangle bounds( world_to_screen( _world_bounding_box.lower( ) ), world_to_screen( _world_bounding_box.upper( ) ) );
...
Camera::HeadUpDisplay::HeadUpDisplay( cdec _center_x_percent,
                                      cdec _center_y_percent,
                                      cdec _width_percent,
                                      cdec _height_percent ) :
    m_center_x_percent( _center_x_percent ),
    m_center_y_percent( _center_y_percent ),
    m_width_percent( _width_percent ),
    m_height_percent( _height_percent )
{
...
FixedRectangle Camera::HeadUpDisplay::bounds( Camera * camera ) const
{
    ...
    Coordinate center = Coordinate( width * ( m_center_x_percent - 0.5 ), height * ( m_center_y_percent - 0.5 ) );
