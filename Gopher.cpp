...
void Gopher::render( )
{
    ...
    if( state( ) == EXPOSED )
    {
        ...
        Polygon inner_fur = Polygon::circle( half( INNER_FUR_WIDTH ) ).stretch( Vector( ZERO, INNER_FUR_HEIGHT / INNER_FUR_WIDTH ) );
        ...
        head_drawing.draw( FUR_COLOR1, Polygon::circle( HEAD_RADIUS ).stretch( Vector( HEAD_X_STRETCH, ZERO ) ) );
        ...
        head_drawing.draw( TEETH_COLOR, Polygon::rectangle( TEETH_WIDTH, TEETH_LENGTH, NOSE_OFFSET + Vector( ZERO, -half( TEETH_LENGTH ) ) ) );
