...
Drawing Mob::debug_overlay( ) const
{
    ...
    debug_overlay.draw( WHITE, Shape::rectangle( health_width + 2.0, health_height + 2.0, offset ) );
    ...
    if( alive( ) )
    {
        ...
        {
            debug_overlay.draw( GREEN, Shape::rectangle( health_width, health_height, offset ) );
        }
        else
        {
            debug_overlay.draw( RED, Shape::rectangle( health_width, health_height, offset ) );
            Coordinate health_offset = offset + VectorX( floor( -health_width.half( ) + ( health_width.half( ) ) * health_percentage ) );
            debug_overlay.draw( GREEN, Shape::rectangle( (int)( health_width * health_percentage ), health_height, health_offset ) );
        }
    }
    else
        debug_overlay.draw( BLACK, Shape::rectangle( health_width, health_height, offset ) );
