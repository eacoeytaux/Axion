...
Object & Object::move( )
{
    ...
    while( ( ( remaining_percentage >= 0.0 ) && !isnan( remaining_percentage ) ) && !stationary( ) )
    {
        update_velocity( );
        ...
        if( solid( ) )
        {
            ...
            if( collied_objects.size( ) )
            {
                ...
                {
                    Coordinate origin;
                    inline bool operator( )( const ObjectCollision & c1, const ObjectCollision & c2 ) { return ( c1.line.c2( ).distance( origin ) < c2.line.c2( ).distance( origin ) ); }
                } collision_sort;
        ...
        movement_percentage = movement.magnitude( ) / velocity.magnitude( );
        remaining_percentage *= ( 1.0 - movement_percentage );
    }
...
Drawing Object::debug_overlay( ) const
{
    ...
    // center / velocity
    debug_overlay.draw( HIT_BOX_COLOR, Shape::circle( HIT_BOX_THICKNESS * 2.0, position( ) ), FILLED );
