...
public:
    ...
    Visible( ufloat z = 1.f, uint layer_position = 0 ) {
        *this = Visible( Drawing( ), z, layer_position );
    }
    Visible( const Drawing & drawing, ufloat z = 1.f, uint layer_position = 0 ) {
        m_drawing = drawing;
        this->z( z );
    ...
    ufloat z( ) const { return m_z; }

protected:
    const Visible & z( ufloat z ) const {
        m_z = z;
        return *this;
    }
    ...
    const Visible & add_light_source( const Coordinate & position, planc distance,
                                      const Color & tint = Color( WHITE ).a( 0.f ),
                                      ufloat flicker = 0.f ) const {
        m_lights.push_back( LightSource( position, distance, tint, flicker ) );
        return *this;
    }
    ...
    const Visible & draw( const Drawing & drawing ) const {
        m_drawing.add( drawing );
        return *this;
    }
    const Visible & draw( const varray<Color> & colors, const Shape & shape,
                          ufloat thickness = FILLED ) const {
        m_drawing.add( colors, shape, thickness );
        return *this;
    }
    const Visible & draw( const Color & color, const Shape & shape,
                          ufloat thickness = FILLED ) const {
        m_drawing.add( color, shape, thickness );
        return *this;
    }
    const Visible & draw( const Color & color1, const Color & color2, const Line & line,
                          ufloat thickness = FILLED ) const {
        m_drawing.add( color1, color2, line, thickness );
        return *this;
    }
    const Visible & draw( const Color & color, const Line & line,
                          ufloat thickness = FILLED ) const {
        m_drawing.add( color, line, thickness );
        return *this;
    }
#ifdef AXN_DEBUG
    const Visible & draw( const Color & color, const Vector & vector, uint arrow_head_length,
                          ufloat thickness ) const {
        m_drawing.add( color, vector, arrow_head_length, thickness );
        return *this;
    }
#endif
    ...
    Visible & border_color ( const Color & color ) { m_drawing.border_color( color ); return *this; }
    Visible & border_width( planc width ) { m_drawing.border_width( width ); return *this; }
    Visible & border ( const Color & color, planc width ) { m_drawing.border( color, width ); return *this; }
...
private:
    mutable ufloat m_z = 1.f;     // 1 = player plane, [0,1) = background, >1 = foreground
    mutable int m_layer_position; // determines show order within layer, higher
                                  // value shows on top of lower values

    mutable varray<LightSource> m_lights;

    mutable Drawing m_drawing;
    mutable bool m_drawing_dirty = true;
    mutable bool m_drawing_always_dirty = false;
