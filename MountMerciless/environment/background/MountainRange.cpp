...
MountainRange::MountainRange( World * world, const Planc _base_bottom, const double _z, const Color & _color ) : Object( world )
{
    ...
    draw( _color, Shape( { Coordinate( START_X, _base_bottom ), Coordinate( START_X, BOTTOM ), Coordinate( END_X, BOTTOM ), Coordinate( END_X, _base_bottom ) } ) );
    ...
    while( mountain_base_right.x( ) < END_X )
    {
        ...
        draw( _color, Shape( { mountain_top, mountain_base_left, mountain_base_right } ) );
    }
}
